...
//		        EditorGUILayout.EndHorizontal();
			...
			if (script.meshes_foldout) {
				EditorGUILayout.BeginHorizontal();
		        	...
		        	if (GUILayout.Button(GUIContent("-",tooltip_text),GUILayout.Width(25)) && script.meshes.Count > 1)
		        	{
		        		if (key.control) {
			        		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
			        			Undo.RecordObject(script,"mesh Erase");
				        	#else
			        			Undo.RegisterUndo(script,"mesh Erase");
			        		#endif
				        	if (!key.shift)
				    		{
	...
	function generate_startup()
	{
			...
			for (var i: int = 0;i < script.meshes.Count;++i) {
				if (script.meshes[i]) {
					if (script.meshes[i].gameObject) {
						if (!script.meshes[i].gameObject.GetComponent.<Collider>()) noColliderReport += script.meshes[i].gameObject.name+"\n";
					}
				}
			}
		...
		#if UNITY_WEBPLAYER
		if (!script.settings.showTerrains) load_raw_heightmaps();
		#endif
